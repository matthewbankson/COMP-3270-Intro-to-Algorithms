   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         for (int i = 0; i < word.length(); i++) {
            ...
            if (i == word.length() - 1) { //at the end of the word
               current.isWord = true;
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         Node.ReverseSubtreeMaxWeightComparator reverse = new Node.ReverseSubtreeMaxWeightComparator();
         PriorityQueue<Node> pq = new PriorityQueue<Node>(reverse);
         ...
         while (pq.size() > 0) {
            current = pq.remove();
            if (current.isWord == true && pq2.size() < k) {
               pq2.add(current);
            }
            else if (current.isWord == true && pq2.size() >= k) {
               if (current.getWeight() > pq2.peek().getWeight()) {
                  pq2.remove();
                  pq2.add(current);
               }
            }
            pq.addAll(current.children.values());
         }
         LinkedList<String> output = new LinkedList<String>();
         
         int numResults = Math.min(k, pq2.size());
         
         for (int i = 0; i < numResults; i++) {
            output.addFirst(pq2.remove().getWord());
         }
      ...
      public String topMatch(String prefix) {
         ...
         if (current.isWord && current.getWeight() == current.mySubtreeMaxWeight) {
            ...
            while (current.isWord == false || current.getWeight() != max) {
               pq.addAll(current.children.values());
               current = pq.remove();
               if (current.isWord && current.getWeight() == max) {
                  return current.getWord();
               }
            }
            return current.getWord();
